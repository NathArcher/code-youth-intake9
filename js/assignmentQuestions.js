...
//create a function with called range that will include 3 number parameters called start,end and step 
function range (start,end,step){
//if start, end or step are underfeined return a empty array
  if (start === undefined || end === undefined || step === undefined) {
    return [];
}
//if step is less than or equal to zero or start is greater than end return empty array
if (step <= 0 || start > end ) {
return [];
}
let result = [];
//for loop which makes the array start with first number of the range, end after the second number that has been defined appears and increases by the third number
...
// 1. One including only the titles of the shows
  let titles = tvShows.map( show => show.title)
  console.log (titles);
// 2. One including objects that include only the title and the rating properties
  let titlesAndRatings = tvShows.map (show => ({title:show.title, rating:show.rating}))
